...
@section('title')
    <title>{{get_string('edit_page') . ' - ' . get_setting('site_name', 'site')}}</title>
@endsection

@section('content')
@section('page_title')
    <h3 class="page-title mbot10">{{get_string('edit_page')}}</h3>
@endsection
<div class="col s12">
    @if(!$errors->isEmpty())
        <span class="wrong-error">* {{get_string('validation_error')}}</span>
    @endif
        {!! Form::open(['method' => 'patch', 'url' => route('admin.page.update', $page->id), 'files' => 'true']) !!}
    <div class="panel">
        ...
        <div class="panel-body">
                                    ...
                                    <div id="lang{{$language->id}}" class="tab-pane {{$language->default ? 'active' : ''}}">
                                        <div class="col l6 m6 s12">
                                            <div class="form-group  {{$errors->has('title.'.$language->id.'') ? 'has-error' : ''}}">
                                                {{Form::text('title['.$language->id.']', $page->content($language->id)->getResults()->title, ['class' => 'form-control', 'placeholder' => get_string('page_title')])}}
                                        ...
                                        <div class="col s12">
                                            {{Form::textarea('body['.$language->id.']', $page->content($language->id)->getResults()->content, ['class' => 'hidden desc-content'])}}
                ...
                <div id="data-panel" class="tab-pane">
                    <div class="col s12">
                         <div class="form-group  {{$errors->has('alias') ? 'has-error' : ''}}">
                            {{Form::text('alias', $page->alias, ['class' => 'form-control', 'placeholder' => get_string('alias')])}}
                    ...
                    <div class="col l6 m4 s6 left left-align">
                        <div class="form-group">
                            <div class="switch">
                                <label>
                                    {{get_string('pending')}}{{Form::checkbox('status', $page->status, null, ['id' => 'activeSwitch', 'class' => 'form-control', $page->status ? 'checked' : ''])}}<span class="lever"></span>{{get_string('active')}}
                                </label>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col clearfix l6 m6 s12 mtop10">
                <div class="form-group">
                    <button class="btn waves-effect" type="submit" name="action">{{get_string('edit_page')}}</button>
                    <a href="{{route('admin.page.index')}}" class="btn waves-effect">{{get_string('page_all')}}</a>
                    <a href="#!" class="delete-button btn waves-effect btn-red" data-id="{{$page->id}}"><i class="material-icons color-white">delete</i></a>
                </div>
            </div>
